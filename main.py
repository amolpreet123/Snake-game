...
pygame.display.set_caption("Snakes with Amol")
...
def welcome():
    ...
    while not exit_game:
        ...
        text_screen("Press Space Bar to Play ! ", white, 235, 290)
        text_screen("(please low down volume ..)" , white , 200 , 340)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                exit_game = True
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE:
                    pygame.mixer.music.load('Timeless.mp3')
...
def gameloop():
    ...
    while not exit_game:
        if game_over:
            ...
            text_screen("Score : "+ str(score), green , 320 , 300)
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    exit_game = True
                if event.type == pygame.KEYDOWN:
